...
export const NavbarSettings: React.FC<{ targetComponentId: string }> = ({
  targetComponentId,
}) => {
  const settings = usePageBuilderStore(
    (state) => state.navbar[targetComponentId] ?? NAVBAR_DEFAULTS,
  );
  const setNavbar = usePageBuilderStore((state) => state.setNavbar);
  ...
  const handleChange = (
    ...
    setNavbar(targetComponentId, { [field]: value });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 pb-4 border-b border-border">
        ...
        <h3 className="font-medium text-foreground">Navbar Settings</h3>
      </div>
          ...
          <div className="space-y-1">
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) =>
                  handleChange('ctaButtonTextColor', e.target.value)
                }
                className="w-8 h-8 rounded cursor-pointer"
              />
              <input
