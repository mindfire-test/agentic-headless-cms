      ...
      switch (fmt) {
        ...
        case 'MMM DD, YYYY': {
          ...
          const mIndex = MONTH_NAMES.findIndex((m) =>
            m.toLowerCase().startsWith(part0.toLowerCase()),
          );
        ...
        case 'DD MMM YYYY': {
          ...
          const part2_0 = parts2[0];
          const part2_1 = parts2[1];
          const part2_2 = parts2[2];
          if (!part2_0 || !part2_1 || !part2_2) return null;
          const mIndex2 = MONTH_NAMES.findIndex((m) =>
            m.toLowerCase().startsWith(part2_1.toLowerCase()),
          );
          if (mIndex2 === -1) return null;
          day = parseInt(part2_0);
          month = mIndex2 + 1;
          year = parseInt(part2_2);
          yearStr = part2_2;
          break;
        }
      }
...
const getDaysInMonth = (date: Date, weekStart: 0 | 1 = 0): Date[] => {
  ...
  const offset =
    weekStart === 1 ? (startDay === 0 ? 6 : startDay - 1) : startDay;
 ...
 */
const getThemeStyles = (themeMode: ThemeMode): ThemeModeStyles =>
  THEME_MODES[themeMode] as ThemeModeStyles;

/**
 * Retrieves color scheme-specific styles for the application
 */
const getColorStyles = (colorScheme: ColorScheme): ColorSchemeStyles =>
  COLOR_SCHEMES[colorScheme] as ColorSchemeStyles;

/**
 * Generates CSS classes for date range highlighting with appropriate opacity
 */
const getInRangeStyle = (
  themeMode: ThemeMode,
  colorScheme: ColorScheme,
): string => {
...
const getPopupPositionClasses = (position: string): string => {
  ...
  return positions[position] || 'top-full right-0 mt-2';
};
...
): node is React.ReactElement<IconProps> => {
              ...
              <Button
                ...
                className={cn(
                  ...
                  isInRange &&
                    !isStart &&
                    !isEnd &&
                    cn(getInRangeStyle(themeMode, colorScheme), 'rounded-xl'),
                  isStart &&
                    cn(
                      'rounded-l-xl bg-gradient-to-r',
                      colorStyles.primary[themeMode],
                      'text-white shadow-sm',
                    ),
                  isEnd &&
                    cn(
                      'rounded-r-xl bg-gradient-to-r',
                      colorStyles.primary[themeMode],
                      'text-white shadow-sm',
                    ),
                  ...
                  isToday &&
                    !isSelected &&
                    !isInRange &&
                    cn(colorStyles.accent[themeMode]),
                  ...
                  !isSelected &&
                    !isInRange &&
                    !isDisabled &&
                    cn('hover:bg-opacity-50', themeStyles.hover),
  ...
  (
    ...
    useEffect(() => {
      if (variant === 'single') {
        if (isDate(value)) {
          ...
          const dateChanged =
            !selectedDate !== !value ||
            (!!selectedDate && !!value && !isSameDay(selectedDate, value));
          if (dateChanged) {
        ...
        if (isDateRange(value)) {
          ...
          const startChanged =
            !selectedRange.start !== !value.start ||
            (!!selectedRange.start &&
              !!value.start &&
              !isSameDay(selectedRange.start, value.start));
          const endChanged =
            !selectedRange.end !== !value.end ||
            (!!selectedRange.end &&
              !!value.end &&
              !isSameDay(selectedRange.end, value.end));
    ...
    const validateDate = (date: Date | null): string | null => {
      ...
      if (date && minDate && date < minDate)
        return `Date must be after ${formatDate(minDate, format)}`;
      if (date && maxDate && date > maxDate)
        return `Date must be before ${formatDate(maxDate, format)}`;
      if (date && disabledDates?.some((d) => isSameDay(d, date)))
    ...
    const handleRangeInputChange = (index: 0 | 1, value: string) => {
      ...
      if (newRange.start && newRange.end) {
        const error = validateOnChange
          ? validateDate(newRange.start) || validateDate(newRange.end)
          : null;
        setInternalError(error);
        onError?.(error);
    ...
    return (
      <div
        ref={containerRef}
        className={cn(
          'relative text-foreground space-y-3',
          themeMode,
          className,
        )}
      >
