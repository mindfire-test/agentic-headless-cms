  ...
  const getAnimationVariants = (): Record<AnimationType, Variants> => {
    ...
    return {
      ...
      reveal: {
        hidden: {
          clipPath: isHorizontal
            ? `inset(0 ${position === 'left' ? '100%' : '0'} 0 ${position === 'right' ? '100%' : '0'})`
            : `inset(${position === 'top' ? '0' : '100%'} 0 ${position === 'bottom' ? '0' : '100%'} 0)`,
      ...
      hinge: {
        hidden: {
          opacity: 0,
          rotateY: isHorizontal ? (position === 'left' ? -90 : 90) : 0,
          rotateX: !isHorizontal ? (position === 'top' ? 90 : -90) : 0,
        ...
        >
          {showOverlay && (
            <motion.div
              ...
              }} // Using rgba for overlay is fine as it needs transparency
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
          ...
          <motion.div
            ...
            style={
              getDrawerDynamicStyles() as import('framer-motion').MotionStyle
            }
