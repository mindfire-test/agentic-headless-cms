...
export interface MaintenancePageProps
  extends MaintenancePageClassProps, VariantProps<typeof containerVariants> {
...
export const MaintenancePage: React.FC<MaintenancePageProps> = React.memo(
  ({
      ...
      () =>
        childrenArray.filter((child) => {
          if (
            ...
            const divElement = child as React.ReactElement<{
              className?: string;
            }>;
            return (
              typeof divElement.props?.className === 'string' &&
              divElement.props.className.includes('absolute')
            );
          }
          return false;
        }),
    ...
    const getComponentName = React.useCallback(
      (type: React.ElementType | string): string => {
        if (typeof type === 'string') return '';
        const component = type as React.ComponentType<unknown> & {
          displayName?: string;
          name?: string;
        };
    ...
    const checkNestedChildren = React.useCallback(
      (children: React.ReactNode, componentName: string): boolean => {
        ...
        return childrenArray.some((nestedChild) => {
          ...
          const nestedDisplayName =
            typeof nestedType !== 'string' ? getComponentName(nestedType) : '';
    ...
    const matchesComponent = React.useCallback(
      (
        ...
        if (displayName === componentName || childType === ComponentType)
          return true;
        ...
        if (typeof childType === 'string' && childType === 'div') {
          ...
          return props.children
            ? checkNestedChildren(props.children, componentName)
            : false;
        }
    ...
    const logoChildren = React.useMemo(() => {
      ...
      return childrenArray.filter((child) =>
        matchesComponent(child, 'MaintenancePageLogo', MaintenancePageLogo),
      );
    ...
    const descriptionChildren = React.useMemo(() => {
      if (!split) return [];
      return childrenArray.filter((child) =>
        matchesComponent(child, 'MaintenancePageDesc', MaintenancePageDesc),
      );
    }, [childrenArray, split, matchesComponent]);
    ...
    const contentChildren = React.useMemo(() => {
      ...
      return filtered.sort((a, b) => {
        ...
        const getOrder = (child: React.ReactElement) => {
          ...
          const displayName = getComponentName(childType);
          if (
            displayName === 'MaintenancePageLogo' ||
            childType === MaintenancePageLogo
          )
            return 0;
          return 1;
        };
    ...
    const iconInstances = React.useMemo(() => {
      ...
      return [
        ...
        {
          size: 20,
          positionClass: 'top-[40%] left-[3%]',
          delay: 2,
          duration: 12,
        },
        {
          size: 32,
          positionClass: 'bottom-[30%] right-[5%]',
          delay: 0.5,
          duration: 9,
        },
        {
        ...
        {
          size: 28,
          positionClass: 'top-[60%] right-[12%]',
          delay: 2.5,
          duration: 13,
        },
        {
          size: 22,
          positionClass: 'bottom-[10%] right-[15%]',
          delay: 3,
          duration: 10,
        },
        {
          size: 26,
          positionClass: 'top-[80%] left-[7%]',
          delay: 1.2,
          duration: 14,
        },
      ];
...
export const MaintenancePageLogo: React.FC<MaintenancePageLogoProps> =
  React.memo(({ logo, companyName, children, className }) => {
    const { variant, backgroundImage } = React.useContext(
      MaintenancePageContext,
    );
    ...
    if (children) {
      return (
        <div
          className={cn(
            'flex flex-col gap-2 mb-6 font-bold',
            alignmentClass,
            className,
          )}
        >
    ...
    return (
      <div
        className={cn('flex flex-row gap-2 mb-6', alignmentClass, className)}
      >
        {renderLogo()}
...
export const MaintenancePageHeading: React.FC<MaintenancePageHeadingProps> =
  React.memo(({ title, children, className }) => {
    const { variant, backgroundImage, split } = React.useContext(
      MaintenancePageContext,
    );
    const isDark = variant === 'dark';
    const hasBackgroundImage = !!backgroundImage;
      ...
      return (
        <div>
          <Typography
            variant="h1"
            className={cn(
              'text-4xl sm:text-5xl lg:text-6xl font-bold mb-4 uppercase tracking-wide',
              alignmentClass,
              textColorClass,
              className,
            )}
...
export const MaintenancePageDesc: React.FC<MaintenancePageDescProps> =
  React.memo(({ description, children, className }) => {
    const { variant, backgroundImage, split } = React.useContext(
      MaintenancePageContext,
    );
    const isDark = variant === 'dark';
...
export const MaintenancePageCountdown: React.FC<MaintenancePageCountdownProps> =
  React.memo(
    ({ targetDate, onCountdownEnd, className, animationType = 'fade' }) => {
      const { variant, backgroundImage, split } = React.useContext(
        MaintenancePageContext,
      );
      const isDark = variant === 'dark';
      ...
      const AnimatedValue = React.useCallback(
          ...
          return (
            ...
            >
              <Typography
                variant="body-small"
                className={cn(
                  'uppercase mb-2 text-xs font-semibold',
                  countdownLabelClass,
                )}
              >
                {label}
...
export const MaintenancePageEmailSubscription: React.FC<MaintenancePageEmailSubscriptionProps> =
  React.memo(
    ({
      ...
      return (
        <div
          className={cn(
            'w-full mb-8',
            split ? 'max-w-2xl mx-auto' : 'max-w-md mx-auto',
            className,
          )}
        >
          <form
          ...
          >
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-2">
              ...
              <Button
                type="submit"
                className={cn(
                  'h-16 sm:h-12 px-6 sm:px-6 py-4 sm:py-0 text-white font-medium whitespace-nowrap text-base sm:text-sm',
                  buttonBgClass,
                )}
              >
                {buttonText}
...
MaintenancePageEmailSubscription.displayName =
  'MaintenancePageEmailSubscription';
...
export const MaintenancePageSocialIcons: React.FC<MaintenancePageSocialIconsProps> =
  React.memo(({ icons, className }) => {
    const { variant, backgroundImage, split } = React.useContext(
      MaintenancePageContext,
    );
    const isDark = variant === 'dark';
...
export const MaintenancePageFooter: React.FC<MaintenancePageClassProps> =
  React.memo(({ children, className }) => {
    ...
    return (
      <div
        className={cn('text-sm text-center w-full', textColorClass, className)}
      >
        {children}
      </div>
    );
